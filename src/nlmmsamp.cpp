...
//' @export
// [[Rcpp::export]]
List nlmmsamp2(arma::mat x, arma::mat z, arma::vec u, arma::vec y, arma::vec clust, arma::vec block,
  arma::vec samples, arma::mat betaprior, arma::vec phivprior, arma::vec psivprior) {
  ...
  arma::mat betasave(samples(0), p, arma::fill::zeros);
  arma::mat psivsave(samples(0), k, arma::fill::zeros);
  arma::mat phivsave(samples(0), q * (q + 1) / 2, arma::fill::zeros);
  ...
  arma::vec zvec(size(y), arma::fill::zeros);
  arma::vec pvec(size(y), arma::fill::ones);
  ...
  for (int i = 0; i < samples(0); i++) {
    ...
    for (int j = 0; j < n; j++) {
      low = indx(j, 0);
      upp = indx(j, 1);
      zeta.row(j) = betapost2(z.rows(low, upp), y(arma::span(low, upp)) - x.rows(low, upp) * beta, 
        pvec(arma::span(low, upp)), mz, Rz).t();
      zvec(arma::span(low, upp)) = z.rows(low, upp) * zeta.row(j).t();
    }
    
    beta = betablockpost2(x, z, y, clust, pvec, Rz, mb, Rb);
    betasave.row(i) = beta.t();
    
    for (int j = 0; j < k; j++) {
      bndx = find(u == j + 1);
      psiv(j) = sigmpost(y(bndx), x.rows(bndx) * beta + zvec(bndx), psivprior(0), psivprior(1));
      psivsave(i, j) = psiv(j);
      pvec(bndx).fill(psiv(j));
    }
    ...
    for (int i = 1; i < (b + 1); i++) {
      bndx = find(block == i);
      y(bndx) = normperm2(samples(1), y(bndx), x.rows(bndx) * beta + zvec(bndx), sqrt(pvec(bndx)));
    }
  }
